...
import {
  ...
  findUserWorkspaces as supabaseFindUserWorkspaces,
  findWorkspaceById as supabaseFindWorkspaceById,
  createWorkspace as supabaseCreateWorkspace,
  createWorkspaceMember as supabaseCreateWorkspaceMember,
  updateUser as supabaseUpdateUser,
  findWorkspaceConfig as supabaseFindWorkspaceConfig,
  isPrismaReachable,
 ...
 * Returns { session, workspaceId, role } if authorized, throws otherwise.
 *
...
export async function requireWorkspaceAccess(
  ...
  const membership = await supabaseFindWorkspaceMember(userId, requestedWorkspaceId)
  if (!membership || !membership.isActive) {
    throw new Error("You do not have access to this workspace")
  }
...
export async function getUserWorkspaces(userId: string) {
  ...
  // Supabase REST API fallback
  return await supabaseFindUserWorkspaces(userId)
}
...
export async function createDefaultWorkspace(userId: string, businessName: string) {
  ...
  // Supabase REST API fallback
  const workspace = await supabaseCreateWorkspace({
    name: businessName,
    slug,
    plan: "FREE",
    settings: JSON.stringify({}),
  ...
  await supabaseCreateWorkspaceMember({
    userId,
    workspaceId: workspace.id,
    role: "OWNER",
  ...
  // Note: workspace_configs may need a separate create, but we skip for simplicity

  await supabaseUpdateUser(userId, { workspaceId: workspace.id })
...
export async function switchWorkspace(userId: string, workspaceId: string) {
  ...
  // Supabase REST API fallback
  const membership = await supabaseFindWorkspaceMember(userId, workspaceId)
  if (!membership || !membership.isActive) {
    throw new Error("You do not have access to this workspace")
  }

  const workspace = await supabaseFindWorkspaceById(workspaceId)
  if (!workspace) {
    throw new Error("Workspace not found")
  }

  await supabaseUpdateUser(userId, { workspaceId })
...
export async function getUserFirstWorkspace(userId: string) {
  ...
  // Supabase REST API fallback
  const membership = await supabaseFindFirstWorkspace(userId)
